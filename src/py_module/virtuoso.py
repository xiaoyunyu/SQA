...
def id_query_out_rel(data_source, node_id, unique = True):
    query = '''
...
def rel_query_obj(relation):
    query = '''
        SELECT ?object FROM <%s> WHERE {?subject <%s> ?object}
    ''' % (data_source,relation)
    json_response = sparql_query(query, query_url)

    try:
        res = [str(item['object']['value']) for item in json_response['results']['bindings']]
        return list(set(res))
    except:
        return []

def partial_str_query_id(string):
    query = '''
        SELECT ?node_id FROM <%s> WHERE  { ?node_id <fb:type.object.ngram> "%s" }
    ''' % (data_source,string)
    json_response = sparql_query(query, query_url)
