...
function SeqRankingLoader:__init(datafile, negSize, negRange, logger)
    ...
    self.rel_Mat = torch.LongTensor(self.negSize+1, self.batchSize)
...
function SeqRankingLoader:nextBatch(circular)
    ...
    self.rel_Mat[1] = self.posMatches[dataIdx]
    self.rel_Mat[{{2,self.negSize+1},{}}] = self.negMatch
    if self.seqLengths ~= nil then
        return self.sequences[dataIdx], self.rel_Mat, self.seqLengths[dataIdx]
    else
        return self.sequences[dataIdx], self.rel_Mat
    end
...
function createSeqRankingData(dataPath, savePath, wordVocab, fbVocab, batchSize)
    ...
    while true do
        ...
        assert(fbVocab:index(fields[2])~=0)
