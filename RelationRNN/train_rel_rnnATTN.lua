...
cmd:option('-back','')
cmd:option('-config','SH+SA')
---------------------------------------------------------------
cmd:option('-syn',false)
...
if not opt.random_init then
    if opt.predicate_mode then
        ...
        loadPretrainedEmbed(relEmbed,'../vocab/relEmbed.300.t7')
    end
else
...
if opt.predicate_mode then
    relBranch = nn.Sequential():add(nn.View(-1)):add(relEmbed):add(nn.Dropout(opt.dropoutRate_r))
                    :add(relLiLayer)
else
    relBranch = nn.Sequential():add(nn.View(-1)):add(relEmbed):add(nn.Dropout(opt.dropoutRate_r))
...
-- for i = 1, 1 do
    ...
    if opt.predicate_mode then
        if opt.syn then
            drel_wMat, drel_rMat,drel_sMat = unpack(relCombLayer:backward({rel_wMat,rel_rMat,rel_sMat},drel_Mat))
