...
-- parse input params
    ...
    local tmnLayer = model.tmnLayer_pos
    --local tmnLayer = model.maxLayer_pos

    local maxLayer_pos =model.maxLayer_pos
    ...
    for i = 1, loader.numBatch do
        ...
        if opt.rnn_mode=='Linear' then
            wrdEmd_smt = seqEmbed:forward(seq)--semantic info
        ...
        if not usePreCal then
            ...
            if opt.predicate_mode then
                ...
                rel_seq = rel_vlayer:forward(rel_em)
            ...
            rel_Mat = rel_Mat:view(-1,loader.batchSize,opt.hiddenSize):transpose(1,2)
        else
        ...
        local relProd =  prodLayer:forward({normMat, rel_Mat}) 
        --print(relProd[5][1]-relProd[5][3])
        if opt.soft then
            relscores = tmnLayer:forward(relProd)
        else
            relscores = maxLayer_pos:forward(relProd)
        end
