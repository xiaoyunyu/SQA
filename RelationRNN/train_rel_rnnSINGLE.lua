...
require '../init3.lua'
require 'gnuplot'
include('test_rel_rnnSINGLE.lua')
...
cmd:option('-qu_norm',true)
cmd:option('-rel_update',true)
cmd:option('-soft_update',false)
---------------------------------------------------------------
cmd:option('-soft',false)
cmd:option('-rnn_mode','GRU')
cmd:option('-attn_mode',false)
cmd:option('-predicate_mode',false)
---------------------------------------------------------------
cmd:option('-config','')
...
cmd:option('-relEmbedSize',300,'size of rel embedding')
...
cmd:option('-wrdEmbedPath','word.glove100k.t7','pretained word embedding path')
...
cmd:option('-dataFile', 'train.relword.150.torch','training data file')
cmd:option('-testData','valid.rel.FB5M.torch','run test on which data set')
cmd:option('-testOn',true)
cmd:option('-dbg',false)
...
local pathDic = parse_path_prefix('savePath')
opt.wrdEmbedPath = pathDic['vocab']..opt.wrdEmbedPath
opt.saveFile = pathDic['model']..opt.saveFile
opt.dataFile = pathDic['data']..opt.dataFile
opt.testData = pathDic['data']..opt.testData

if opt.predicate_mode then 
    opt.relEmbedSize=256
end
----------------------------- define loader -----------------------------
...
if not opt.random_init then
    loadPretrainedEmbed(relEmbed,'../vocab/relEmbed.'..opt.relEmbedSize..'.t7')
else
...
-- loadPretrainedEmbed(rel2word,'../vocab/rel2word.t7')
-- -- rel word embedding model
local relWordEmbed = cudacheck(nn.LookupTable(opt.relWordSize, opt.wrdEmbedSize))
loadPretrainedEmbed(relWordEmbed,'../vocab/relWord_embed.t7')
...
seqModel:add(linearLayer):add(nn.Transpose({1,2}))
----------------------------attn model--------------------------------
...
local RNN = {}
if opt.rnn_mode=='GRU' then
    RNN[1] = BiGRU(config_s)--v
elseif opt.rnn_mode=='LSTM' then
    RNN[1] = BiLSTM(config_s)--v
elseif opt.rnn_mode == 'Linear' then
    RNN[1]=Linear(opt.wrdEmbedSize,opt.hiddenSize,false,false)
end
RNN[2] = RNN[1]:clone()--q
RNN[3] = RNN[1]:clone()--k
...
if opt.rnn_mode == 'Linear' then
    qLiLayer = Linear(opt.hiddenSize, 1, true,false)
     kLiLayer = Linear(opt.hiddenSize, 1, true,false)
     vLiLayer = Linear(opt.hiddenSize, opt.hiddenSize, false)
else
     qLiLayer = Linear(opt.hiddenSize*2, 1, true,false)
     kLiLayer = Linear(opt.hiddenSize*2, 1, true,false)
     vLiLayer = Linear(2 * opt.hiddenSize, opt.hiddenSize, false)
end
...
local normLayer = nn.Sequential():add(nn.View(-1,opt.hiddenSize))
if opt.qu_norm then
    normLayer:add(nn.Normalize(2,1e-6))
end
-- ------VAE layer-----
...
if opt.predicate_mode then
    relBranch = nn.Sequential():add(nn.View(-1)):add(relEmbed):add(nn.Dropout(opt.dropoutRate_r)):add(relLiLayer)--:add(nn.ReLU(true))--[(33*batchsize) * hiddenSize]
else
    relBranch = nn.Sequential():add(nn.View(-1)):add(relEmbed):add(nn.Dropout(opt.dropoutRate_r)):add(nn.Normalize(2,1e-6))
end
local relCombLayer = nn.Sequential():add(BatchComb()):add(nn.Normalize(2,1e-6))
...
local maxLayer_pos = nn.Mean(3)
local maxLayer_neg = nn.Mean(3)
...
if opt.soft_update then
    optimParams[#optimParams+1], optimGrad[#optimGrad+1] = pospoolLayer:getParameters()--10
    pospool_paramID = #optimParams
end
...
-- for i = 1, 1 do
    ...
    negpoolLayer:zeroGradParameters()
    ----------------------- load minibatch ------------------------
    ...
    debug_size('seq',seq)
    if opt.rnn_mode=='Linear' then
        wrdEmd_smt = wordEmbed:forward(seq)--semantic info
        local seq_pos = torch.range(1,seq:size(1))
        seq_pos = torch.repeatTensor(seq_pos,seq:size(2),1):transpose(1,2)
        wrdEmd_pos = relposEmbed:forward(seq_pos)
        wrdEmd = wrdEmd_smt+wrdEmd_pos
    else
        wrdEmd = wordEmbed:forward(seq)
    end
    if not opt.attn_mode then
        attnMat = seqModel:forward(wrdEmd)
    else 
    ...
    if opt.predicate_mode then
        ...
        debug_size('rel_r',rel_r)
        rel_rMat = relBranch:forward(rel_r)
        debug_size('rel_Mat',rel_Mat)
        rel_Mat = relCombLayer:forward({rel_wMat,rel_rMat})
    else
        debug_size('rel_r',rel_r)
        rel_Mat = relBranch:forward(rel_r)
        debug_size('rel_Mat',rel_Mat)
    end
    ...
    local posOut = rel_Mat:view(loader.negSize+1,loader.batchSize,-1):narrow(1,1,1):transpose(1,2)
    local negOut = rel_Mat:view(loader.negSize+1,loader.batchSize,-1):narrow(1,2,loader.negSize):transpose(1,2)
    debug_size('posOut',posOut)
    debug_size('negOut',negOut)
    -----------trunk----------------------------
    -- posProd[b * 1 * l] negProd[b * negSize * l]
    local posProd,negProd = unpack(prodLayer:forward({normMat, posOut,negOut}) )
    ...
    --score
    local scores = scoreModel:forward({posPool,negPool})
    --loss
    ...
    if opt.soft then
        ...
        pospoolLayer.gradWeight:add(negpoolLayer.gradWeight)
        if opt.soft_update then
            gradNorm = gradNorm + optimGrad[possum_paramID]:norm()^2+optimGrad[pospool_paramID]:norm()^2
        else
    ...
    local dnormMat,dposOut,dnegOut = unpack( prodLayer:backward({normMat, posOut,negOut},{dposProd,dnegProd} ) )
    ----------------------relation branch-------------
    local drel_Mat = torch.Tensor(loader.negSize+1,loader.batchSize,opt.hiddenSize)
    ...
    -- gradNorm = gradNorm + optimGrad[relk_paramID]:norm()^2
        ...
        relBranch:backward(rel_r,drel_Mat)
        gradNorm = gradNorm + relEmbed.gradWeight:norm()^2
    end
    -- -------------latent layer------------------------
    ...
    if opt.attn_mode then
        ...
        dwrdEmd = seqModel:backward(wrdEmd,dattnMat)
    end
    ...
    if opt.rel_update then
        relEmbed.gradWeight[opt.relSize]:zero()
        relEmbed:updateParameters(lrRel / (1 + 0.0001 * i))
        if not opt.predicate_mode then
            -- relEmbed.weight:renorm(2, 2, 1)--在relsize维度上归一化，而不是embedding归一化！
        end
    end
    ...
    negsumLayer.weight:copy(possumLayer.weight)
    negsumLayer.bias:copy(possumLayer.bias)
    negpoolLayer.weight:copy(pospoolLayer.weight)
    ...
    if i % loader.numBatch == 0 then
        local now = os.time()
        print(string.format('cost time is %f',(now-last)))
        flog.info(string.format('cost time is %f',(now-last)))
        last = now
    -- if true then
        ...
        if opt.testOn  then
            ...
            table.insert(plotLoss,testLoss[1]/testLoss[2])
            local xTensor = torch.range(1,#plotLoss)
            local yTensor = torch.Tensor(plotLoss)
            gnuplot.pngfigure(opt.logFile..curr_time..'.png')
            gnuplot.plot('currCurve',xTensor,yTensor,'~')
