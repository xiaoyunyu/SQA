...
local RNN = {}
if opt.rnn == 'cudnn.gru' then
	RNN[1] = cudnn.BGRU(opt.wrdEmbedSize,opt.hiddenSize,1,false)--v
	RNN[2] = cudnn.BGRU(opt.wrdEmbedSize,opt.hiddenSize,1,false)--q
	RNN[3] = cudnn.BGRU(opt.wrdEmbedSize,opt.hiddenSize,1,false)--k
	elseif opt.rnn == 'cudnn.lstm' then
	RNN[1] = cudnn.BLSTM(opt.wrdEmbedSize,opt.hiddenSize,1,false)--v
	RNN[2] = cudnn.BLSTM(opt.wrdEmbedSize,opt.hiddenSize,1,false)--q
	RNN[3] = cudnn.BLSTM(opt.wrdEmbedSize,opt.hiddenSize,1,false)--k
else
	local config_s = {}
...
-- for i = 1, 1 do
    ...
    local attnMat = attnModel:forward({qMat,kMat,vMat}) 
    ...
    local rel_wMat = rel_vlayer:forward(rel_em)
    ...
    local posPool = tmnLayer_pos:forward(posProd)
    local negPool = tmnLayer_neg:forward(negProd)
    --local relPool = tmnLayer:forward(relProd)
    -- local posPool = relPool:narrow(2,1,1)
    -- local negPool = relPool:narrow(2,2,loader.negSize)

    -- local posPool = maxLayer_pos:forward(posProd)
    -- local negPool = maxLayer_neg:forward(negProd)
    --score
    ...
    local dposProd = tmnLayer_pos:backward(posProd,dposPool)
    local dnegProd = tmnLayer_neg:backward(negProd,dnegPool)
    possumLayer.gradWeight:add(negsumLayer.gradWeight) 
    possumLayer.gradBias:add(negsumLayer.gradBias)

    -- local dposProd = maxLayer_pos:backward(posProd,dposPool)
    -- local dnegProd = maxLayer_neg:backward(negProd,dnegPool)
    ...
    local dattnMat = normLayer:backward(attnMat,dnormMat)
    -- local dattnMat = ffnLayer:backward(attnMat,dffnMat)
    -- dattnMat = dattnMat + dffnMat
    -- gradNorm = gradNorm + optimGrad[ffn_paramID]:norm()^2
    local dqMat,dkMat,dvMat = unpack( attnModel:backward({qMat,kMat,vMat},dattnMat) )
    -- dvMat:add(dvMat_lat)
    ...
    local dwrdEmd = v:backward(wrdEmd,dvMat)
