...
cmd:option('-wordEmbedPath','../vocab/word.glove100k.t7','pretained word embedding path')
...
cmd:option('-saveFile','/home/yxy/base/CFO/CRF/model.BiGRU','filename for saving trained model')
...
for i = 1, maxIters do
    ...
    if i % (loader.numBatch * opt.saveEvery) == 0 then
        ...
        local str = opt.saveFile..'.'..opt.useGPU..'.'..epoch
        local prec = test_model(model)
        flog.info(string.format("precision of %s is %5f", str, prec))
        if prec > maxPrecision then
            print('Saving model after epoch ' .. epoch)
            maxPrecision=prec
            optName = opt.saveFile..'.'..opt.useGPU..'*'
            torch.save(optName, model)
        end
    end
end
